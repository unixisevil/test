                      ###mesos:一个数据中心里, 能细粒度资源分享的平台

我们提出mesos, 一个能在多种不同集群计算框架(例如hadoop,mpi等)之间共享集群的平台.
共享改善了集群的资源利用,避免了每框架应用数据的复制.mesos能以细粒度方式分享资源, 允许框架通过轮流读取每个机器上的数据, 来完成数据局部性.为了支持当今框架使用日益复杂的调度器, mesos 引入了叫做resource offers的分布式双层调度机制.
